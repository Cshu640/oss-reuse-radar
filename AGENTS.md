...
- 代码和模型平台的正常浏览器路径必须使用本地同源 `/api/upstream/*`；外部链接只作为明确的搜索回退入口。
- 上游请求必须经过统一的超时、有限重试、缓存/条件请求、in-flight 去重、供应商隔离、降级和脱敏响应；缓存只存内存，禁止保存 Authorization、Token 或环境变量。
- `GITHUB_TOKEN` 只能由服务端进程读取；健康检查、浏览器 API、日志、缓存、测试快照和静态资源不得出现真实 Token。
- GitHub primary/secondary rate limit 必须分别处理；剩余量为零时等待 reset，`Retry-After` 必须遵守，无限重试和高频轰炸均禁止。
- OSS-0Q 的自动化测试必须使用可注入 fetch、clock、sleeper 和假 Token，覆盖缓存、重试、限流、降级、密钥边界和有效空结果；真实 Windows 上游验收必须单独记录，不得用本地模拟代替。
