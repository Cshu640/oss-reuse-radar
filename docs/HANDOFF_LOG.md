...
## 2026-08-11 — OSS-0Q public beta upstream hardening

- Current phase: OSS-0Q — server-side upstream gateway implementation; full live Windows acceptance remains open.
- Git: branch `oss/phase-0-public-readiness`; OSS-0P freeze commit `f2a10a7`; feature commit `270961e`; no remote, tag, push, merge, release, or public repository.
- Scout: native Node Fetch/AbortController plus small in-repository bounded limiter/cache adopted. Octokit and undici are reference-only. p-limit, p-queue, Bottleneck, fetch-retry, and ky are rejected for now. No upstream source copied or vendored. Full reasons and licenses are in `docs/UPSTREAM_GATEWAY_DECISION.md` and `docs/SOURCE_LEDGER.json`.
- Completed: browser code/model normal path now uses local same-origin `/api/upstream/search` and `/api/upstream/radar`; server gateway provides memory-only bounded cache, in-flight dedupe, ETag/Last-Modified, timeout, bounded retry/backoff, Retry-After, GitHub reset cooldown, provider/global concurrency, optional server-only GitHub auth, sanitized contract, package integration, Gitee fallback-only contract, and UI status labels for anonymous/cache/stale/degraded states.
- Tests: Node `v24.18.0` `node --test tests/*.mjs` passed 30/30; gateway tests passed 20/20; JS/MJS syntax passed; JSON parse passed for manifest/state/source ledger; `git diff --check` passed; local HTTP server smoke returned health/home/upstream/Gitee HTTP 200. Playwright Python mock not run because bundled Python lacks `playwright`.
- Cache/rate-limit evidence: fresh hit, ETag/304 revalidation, in-flight dedupe, bounded eviction, stale-if-error, stale max-age refusal, timeout abort, bounded 5xx retry, 429 Retry-After, GitHub primary zero cooldown, secondary Retry-After, provider isolation, and ordinary 4xx no-retry all passed with injected fakes.
- Secret evidence: static token scan and browser direct-upstream scan passed. A fake token was used only in injected tests; it was absent from gateway response/status/cache. `.env.example` has an empty `GITHUB_TOKEN=` placeholder only.
- Risk/readiness: browser direct-call risk mitigated for normal path; GitHub limits, package reliability, and service instability partially mitigated; Gitee ambiguity mitigated; public governance/adoption remains open. Readiness remains `50/100 — NO`; no adoption claim or score increase was made.
- Not completed/blockers: full Windows live upstream acceptance; natural-time growth; public repository/release/issue/PR/adoption evidence; standalone Playwright mock; shell outbound upstream access is restricted. Memory cache restart invalidation is documented and intentional.
...
- Branch/HEAD at the final evidence snapshot: `oss/phase-0-public-readiness` / `c3eea98`; OSS-0Q checkpoint commit `fe68af1`; harness commit `a30a06f`; docs commit `c3eea98`. No remote, tag, push, merge, release, or public repository.
...
- Workspace: final docs commit `c3eea98` is committed; the subsequent state/handoff/log evidence snapshot is intentionally dirty for audit. No staged files and no runtime data deletion.
...
## 2026-08-11 — OSS-0Q.1 Windows live upstream acceptance passed
...
- Environment: ordinary Windows PowerShell (user run) with
  ...
  Asia/Shanghai; harness `scripts/live-upstream-acceptance.mjs` via
  `RUN_LIVE_UPSTREAM_ACCEPTANCE.cmd` on an ephemeral port; only its own child
  server was started/stopped; GitHub forced anonymous.
- Live matrix: GitHub/Hugging Face/GitLab/ModelScope `PASS_LIVE` (3/3/3/1
  results), Codeberg `PASS_LIVE_EMPTY` (0), npm/`axios` `PASS_LIVE` (3),
  PyPI/`requests` `PASS_LIVE` (1), crates/`serde` `PASS_LIVE` (3), Gitee
  `DEGRADED_FALLBACK` (external search, 0). GitHub anonymous rate-limit
  headers observed (`resource=search`, limit 10, remaining 5, used 5, reset
  observed). Live cache re-hit verified for GitHub, Hugging Face, and npm
  (fresh/fresh, sameData=true; npm second hit faster).
- Tests: Node 30/30; JS/MJS syntax passed; `git diff --check` passed; JSON
  parse passed; sanitized artifact contains no token/header/raw env/private
  path; Python Playwright mock not run (missing `playwright`).
- Completed: real Windows live acceptance; release-blocking judgment;
  documentation commit. Not completed (non-blocking): authenticated GitHub,
  new browser live smoke on this network, standalone Playwright, natural-time
  growth, and public maintenance/adoption evidence.
- Release judgment: no new release blocker; `PUBLIC_BETA_RELEASE_GATE_READY =
  true`; readiness stays `50/100 — NO` with no adoption/public-maintenance
  points added.
- Forbidden honored: no product feature change, paid API, new platform, token
  exposure, direct browser provider call, fake PASS, automatic Codex launch,
  public repo, remote, push, tag, merge, release, or broad redesign.
- Workspace: one independent local docs commit was created; final
  state/handoff/log evidence snapshot intentionally dirty for audit; no
  runtime data deleted; `artifacts/` remains ignored.
- Unique next task: **Public Beta Release Gate**.
...
## 2026-08-11 — OSS-0R Public Beta Release Gate
...
- Secret audit: `git grep` over all 36 reachable commits found no real
  credential; the only hit is the deliberately fake
  `ghp_fake_server_only_1234567890` in `tests/upstream_gateway_test.mjs`.
- Privacy audit: no personal emails or credentials; one early provenance path
  with the Windows account name (now generalized in
  `docs/ARTIFACT_PROVENANCE.md`) is a required user decision because history
  rewrite is not performed without approval.
- Provenance: genuine recovered history preserved (`fd44b7f`, `e60b3a0`); no
  backdating, synthetic tags, or fake contributors; author identities are
  non-personal.
- License/source: `LICENSE` completed to the standard MIT text; source ledger
  confirms `copied_upstream_code: false`; no NOTICE required for current
  composition.
- Naming: `oss-reuse-radar` recommended, display name `OpenRadar`, no rename
  required before public; not a trademark/legal guarantee.
- README/quick-start/CI/community: README.en.md now includes Requirements,
  generic proxy guidance, and current beta limitations; CI uses Node 24 and
  installs Playwright/Chromium; browser mock skips cleanly when Playwright is
  missing; SECURITY.md contact is a user decision (Private Vulnerability
  Reporting recommended).
- Release candidate: `docs/PUBLIC_RELEASE_GATE.md`,
  `docs/PUBLIC_RELEASE_MANIFEST.json` (`public_release_go: true`),
  `docs/GITHUB_PUBLIC_REPOSITORY_PLAN.md`, `docs/RELEASE_NOTES_v0.1.0.md`,
  `docs/PUBLIC_NAMING_DECISION.md`, and generalized
  `docs/ARTIFACT_PROVENANCE.md` created/updated.
- Tests: Node 30/30; JS/MJS syntax passed; Python `py_compile` passed; browser
  mock printed a clean SKIPPED locally (playwright missing; CI installs it);
  JSON parse passed for state and manifest; `git diff --check` passed; real
  Windows live acceptance from 2026-08-11 reused because no runtime code
  changed.
- Known issues/risks: early provenance history path exposure; screenshots
  pending; Gitee fallback-only; memory cache; authenticated GitHub untested;
  no public adoption/maintenance evidence; readiness stays `50/100 — NO`.
- Release judgment: no release blocker remains; `PUBLIC_RELEASE_GO = true`
  pending explicit user approval. Actual publication (create `oss-reuse-radar`,
  push, `main`, tag `v0.1.0`, Release, settings, CI verification) is the next
  task and must not run without approval.
- Forbidden honored: no remote, push, tag, merge, release, public repo,
  website, Codex-for-OSS submission, paid API, new platform, or fake evidence.
- Workspace: final release-candidate docs commit created; evidence trio is
  intentionally dirty for audit; `tests/__pycache__` generated by the compile
  check was removed and Python bytecode is now ignored; no runtime data
  deleted.
