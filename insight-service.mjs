...
export function ruleBasedInsight(projectValue, reason = '') {
  ...
  const subject = project.description || `${project.category || '相关领域'}开源项目`;
  const summary = `${project.name || '这个项目'}是一个${project.category ? `偏${project.category}的` : ''}开源项目，主要用途是：${subject.replace(/[。.!！]+$/u, '')}。`;
  const requirements = [
  ...
  return {
    ...
    fitForUser: `与“优先复用开源、由Codex实施、关注游戏/AI/办公/生活工具”的工作方式存在${project.useTypes.includes('codex') || project.useTypes.includes('business') ? '较高' : '一定'}匹配度；是否值得现在投入仍取决于接入成本和当前主线。`,
    risks,
...
function buildPrompt(project, readme) {
  ...
  return [
    ...
    '用户偏好：Windows电脑、NVIDIA 8GB显存；优先免费开源、可自托管、可由Codex实施；关注游戏开发、AI图片视频音频、Agent/MCP、办公效率、生活工具、Web/App/微信生态；尽量复用现有项目而不是从零开发。',
...
function mergeInsight(project, value, { model, fingerprint, readmeUsed, now }) {
  const fallback = ruleBasedInsight(project);
...
export function createInsightService({
  ...
  async function runGenerate(projectValue, { force = false } = {}) {
    const project = normalizeProjectInput(projectValue);
    ...
    if (!currentStatus.available) {
      return {
        ...ruleBasedInsight(project, currentStatus.message),
    ...
    const response = await fetchWithTimeout(fetchImpl, `${baseUrl}/api/chat`, {
      ...
      body: JSON.stringify({
        model,
        messages: [
          { role: 'system', content: '你是严谨的开源项目分析师，只依据给定材料生成结构化中文结论。' },
          { role: 'user', content: buildPrompt(project, readme) },
        ],
        stream: false,
        think: false,
        keep_alive: 0,
    ...
    const insight = mergeInsight(project, parsed, { model, fingerprint, readmeUsed: Boolean(readme), now });
    const saved = await store.set(project.id, insight);
